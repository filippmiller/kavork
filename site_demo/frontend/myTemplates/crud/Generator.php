...
namespace frontend\myTemplates\crud;
...
class Generator extends \yii\gii\Generator
{
  ...
  public $viewPath;
  public $baseControllerClass = 'frontend\components\Controller';
  ...
  public function hints()
  {
    return array_merge(parent::hints(), [
        ...
        'baseControllerClass' => 'This is the class that the new CRUD controller class will extend from.
                You should provide a fully qualified class name, e.g., <code>frontend\components\Controller</code>.',
  ...
  function getColumnRule($name, $type, $arr, $type_this)
  {
    ...
    if (strpos($name, 'franchisee_id') !== false) return $arr;
    if (in_array($name, ['id', 'franchisee_id', 'last_task', 'visitor_id'])) return $arr;
    ...
    switch ($type) {
      ...
      case Schema::TYPE_MONEY:
        ...
        $this->rangeConditions_num[$name . '_from'][] = "->andFilterWhere(['>=', '{$name}', (float)\$this->{$name}_from])";
        $this->rangeConditions_num[$name . '_from'][] = "->andFilterWhere(['<=', '{$name}', (float)\$this->{$name}_to])";
        break;
      case Schema::TYPE_DATE:
      case Schema::TYPE_TIME:
      case Schema::TYPE_DATETIME:
      case Schema::TYPE_TIMESTAMP:
        ...
        $dt = Schema::TYPE_DATE ? ',' . '\'P1D\'' : '';
        $this->rangeConditions[$name . '_from'][] = "->andFilterWhere(['>=', '{$name}', GridHelper::getDbDateFromDateRangeFormat(\$this->{$name}_from)])";
        $this->rangeConditions[$name . '_from'][] = "->andFilterWhere(['<=', '{$name}', GridHelper::getDbDateFromDateRangeFormat(\$this->{$name}_to" . $dt . ")])";
        $arr['match']['/^.+\s\-\s.+$/'][] = $name;
        break;
    };
    return $arr;
  ...
  public function generateSearchConditions()
  {
    ...
    if (!empty($this->rangeConditions_num)) {
      foreach ($this->rangeConditions_num as $k => $range) {
        $conditions[] = "
        //Filter for ranger {$k}
